...
OR manually via API:
```bash
curl -X POST "https://restart-auction.preview.emergentagent.com/api/scoring/YOUR_LEAGUE_ID/ingest" \
...
```bash
# Via API
curl "https://restart-auction.preview.emergentagent.com/api/scoring/YOUR_LEAGUE_ID/leaderboard" | jq
