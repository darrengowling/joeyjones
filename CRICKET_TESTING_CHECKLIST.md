...
OR manually via API:
```bash
curl -X POST "https://prod-auction-fix.preview.emergentagent.com/api/scoring/YOUR_LEAGUE_ID/ingest" \
...
```bash
# Via API
curl "https://prod-auction-fix.preview.emergentagent.com/api/scoring/YOUR_LEAGUE_ID/leaderboard" | jq
