...
| 6 | **ISSUE-027** | **Fixture Score Update Matching Bug** - Update scores can match wrong fixture by team name | `update_fixture_scores` matches by team name regex without date validation; historical results from API can overwrite future fixtures | Add date filter (±1 day) to fixture matching query in `/fixtures/update-scores` endpoint | 🟡 Medium | Prevents incorrect scores being applied to wrong fixtures |
| 7 | **ISSUE-001** | **Manual Score Entry UI** - No UI for manual score updates | Backend exists, frontend not built | Build score entry UI in CompetitionDashboard | 🟢 Low | Commissioners can fix scores without CSV |
