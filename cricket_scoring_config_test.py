...
class CricketScoringConfigTester:
    ...
    def test_scoring_overrides_endpoint_validation(self) -> bool:
        ...
        if "error" not in result:
            self.log(f"Missing rules validation failed. Expected 400 error but got success", "ERROR")
            return False
        if "Invalid or missing rule" not in detail:
    ...
    def test_non_cricket_league_rejection(self) -> bool:
        ...
        detail = result.get("detail", "")
        if "error" not in result or "only supported for cricket leagues" not in detail:
