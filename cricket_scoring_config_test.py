...
class CricketScoringConfigTester:
    ...
    def test_scoring_overrides_endpoint_validation(self) -> bool:
        ...
        if "error" not in result or "Invalid or missing rule: catch" not in result.get("detail", ""):
            self.log(f"Missing rules validation failed. Expected 'Invalid or missing rule: catch', got: {result.get('detail')}", "ERROR")
            return False
    ...
    def test_non_cricket_league_rejection(self) -> bool:
        ...
        if "error" not in result or "Scoring overrides are only supported for cricket leagues" not in result.get("detail", ""):
            self.log(f"Non-cricket league rejection failed. Expected 'Scoring overrides are only supported for cricket leagues', got: {result.get('detail')}", "ERROR")
            return False
