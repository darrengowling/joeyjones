...
class CricketScoringConfigTester:
    ...
    def test_scoring_overrides_endpoint_validation(self) -> bool:
        ...
        if "error" not in result or "Invalid or missing rule" not in result.get("detail", ""):
            self.log("Missing rules validation failed", "ERROR")
            return False
...
match5,player7,60,1,0,0,0"""
        ...
        updated_overrides = {
            "scoringOverrides": {
                ...
                "milestones": {
                    "halfCentury": {"enabled": True, "points": 20},  # Doubled
                    "century": {"enabled": True, "points": 40},
                    "fiveWicketHaul": {"enabled": True, "points": 60}
                }
            }
        }
