  ...
  if (auction?.status === "waiting") {
    ...
    const handleBeginAuction = async () => {
      try {
        ...
        // Reload auction to get new status
        loadAuction();
      } catch (error) {
      ...
      <div className="min-h-screen bg-gray-100 p-4">
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-lg shadow-lg p-8 text-center">
            <h1 className="text-3xl font-bold mb-4">⏳ Auction Waiting Room</h1>
            <p className="text-lg text-gray-600 mb-6">
              Auction has been created. Waiting for commissioner to begin.
            </p>
            
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
              <h3 className="font-bold mb-2">Participants in Room:</h3>
              <div className="flex flex-wrap justify-center gap-2">
                {participants.map(p => (
                  <div key={p.userId} className="flex items-center gap-2 bg-white px-3 py-1 rounded-full border">
            ...
            {isCommissioner ? (
              ...
              <div>
                <p className="text-gray-600 font-medium">
                  Waiting for commissioner to start the auction...
                </p>
                <p className="text-sm text-gray-500 mt-2">
                  The first team will appear once the auction begins
                </p>
              </div>
            )}
          </div>
