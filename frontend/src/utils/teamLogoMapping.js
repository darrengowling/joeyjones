// Team name to logo file mapping
// Logos stored in /public/assets/clubs/{sport}/
// 
// STANDARDIZED: Uses official Football-Data.org API names
// After DB standardization, only these names are needed

export const footballLogoMapping = {
  // === PREMIER LEAGUE ===
  "Arsenal FC": "arsenal_fc.png",
  "Aston Villa FC": "aston_villa.png",
  "AFC Bournemouth": "afc_bournemouth.png",
  "Brentford FC": "brentford_fc.png",
  "Brighton & Hove Albion FC": "brighton_&_hove_albion.png",
  "Burnley FC": "burnley_fc.png",
  "Chelsea FC": "chelsea_fc.png",
  "Crystal Palace FC": "crystal_palace.png",
  "Everton FC": "everton_fc.png",
  "Fulham FC": "fulham_fc.png",
  "Leeds United FC": "leeds_united.png",
  "Liverpool FC": "liverpool_fc.png",
  "Manchester City FC": "manchester_city.png",
  "Manchester United FC": "manchester_united.png",
  "Newcastle United FC": "newcastle_united.png",
  "Nottingham Forest FC": "nottingham_forest.png",
  "Sunderland AFC": "sunderland_afc.png",
  "Tottenham Hotspur FC": "tottenham_hotspur.png",
  "West Ham United FC": "west_ham_united.png",
  "Wolverhampton Wanderers FC": "wolverhampton_wanderers.png",
  
  // === SPAIN (LA LIGA) ===
  "Real Madrid CF": "real_madrid.png",
  "FC Barcelona": "fc_barcelona.png",
  "Club Atlético de Madrid": "atletico_madrid.png",
  "Athletic Club": "athletic_bilbao.png",
  "Villarreal CF": "villarreal_cf.png",
  "Sevilla FC": "sevilla_fc.png",
  "Real Sociedad": "real_sociedad.png",
  "Girona FC": "girona_fc.png",
  
  // === GERMANY (BUNDESLIGA) ===
  "FC Bayern München": "bayern_munich.png",
  "Borussia Dortmund": "borussia_dortmund.png",
  "Bayer 04 Leverkusen": "bayer_04_leverkusen.png",
  "RB Leipzig": "rb_leipzig.png",
  "VfB Stuttgart": "vfb_stuttgart.png",
  "Eintracht Frankfurt": "eintracht_frankfurt.png",
  
  // === ITALY (SERIE A) ===
  "FC Internazionale Milano": "inter_milan.png",
  "AC Milan": "ac_milan.png",
  "Juventus FC": "juventus_fc.png",
  "Atalanta BC": "atalanta_bc.png",
  "SSC Napoli": "ssc_napoli.png",
  "SS Lazio": "ss_lazio.png",
  "AS Roma": "as_roma.png",
  "Bologna FC 1909": "bologna_fc_1909.png",
  
  // === FRANCE (LIGUE 1) ===
  "Paris Saint-Germain FC": "paris_saint-germain.png",
  "AS Monaco FC": "as_monaco.png",
  "Olympique de Marseille": "olympique_marseille.png",
  "LOSC Lille": "losc_lille.png",
  "Olympique Lyon": "olympique_lyon.png",
  "OGC Nice": "ogc_nice.png",
  "Stade Brestois 29": "stade_brestois_29.png",
  
  // === PORTUGAL (PRIMEIRA LIGA) ===
  "Sport Lisboa e Benfica": "sl_benfica.png",
  "Sporting Clube de Portugal": "sporting_cp.png",
  "FC Porto": "fc_porto.png",
  "SC Braga": "sc_braga.png",
  
  // === NETHERLANDS (EREDIVISIE) ===
  "AFC Ajax": "ajax_amsterdam.png",
  "PSV": "psv_eindhoven.png",
  "Feyenoord Rotterdam": "feyenoord_rotterdam.png",
  
  // === BELGIUM (PRO LEAGUE) ===
  "Club Brugge KV": "club_brugge.png",
  "Royale Union Saint-Gilloise": "union_saint_gilloise.png",
  
  // === SCOTLAND (SPFL) ===
  "Celtic FC": "celtic_fc.png",
  "Celtic": "celtic_fc.png",
  "Rangers FC": "rangers_fc.png",
  "Rangers": "rangers_fc.png",
  
  // === OTHER EUROPEAN LEAGUES ===
  "SK Sturm Graz": "sturm_graz.png",
  "Sturm Graz": "sturm_graz.png",
  "AC Sparta Praha": "sparta_prague.png",
  "Sparta Prague": "sparta_prague.png",
  "GNK Dinamo Zagreb": "dinamo_zagreb.png",
  "Dinamo Zagreb": "dinamo_zagreb.png",
  "BSC Young Boys": "young_boys.png",
  "Young Boys": "young_boys.png",
  "FK Crvena Zvezda": "red_star_belgrade.png",
  "Red Star Belgrade": "red_star_belgrade.png",
  "FC Shakhtar Donetsk": "shakhtar_donetsk.png",
  "Shakhtar Donetsk": "shakhtar_donetsk.png",
  "Jagiellonia Białystok": "jagiellonia_bialystok.png",
  
  // === UEFA CHAMPIONS LEAGUE 2025/26 (Additional Teams) ===
  "FK Bodø/Glimt": "bodo_glimt.png",
  "Galatasaray SK": "galatasaray.png",
  "PAE Olympiakos SFP": "olympiacos.png",
  "Qarabağ Ağdam FK": "fk_qarabag.png",
  "FC København": "fc_kobenhavn.png",
  "Paphos FC": "pafos_fc.png",
  "SK Slavia Praha": "slavia_prague.png",
  
  // === BACKWARD COMPATIBILITY ===
  // Short names for transition period (can remove after full standardization)
  "Real Madrid": "real_madrid.png",
  "Barcelona": "fc_barcelona.png",
  "Atlético Madrid": "atletico_madrid.png",
  "Athletic Bilbao": "athletic_bilbao.png",
  "Manchester City": "manchester_city.png",
  "Arsenal": "arsenal_fc.png",
  "Liverpool": "liverpool_fc.png",
  "Aston Villa": "aston_villa.png",
  "Tottenham": "tottenham_hotspur.png",
  "Tottenham Hotspur": "tottenham_hotspur.png",
  "Newcastle": "newcastle_united.png",
  "Newcastle United": "newcastle_united.png",
  "Chelsea": "chelsea_fc.png",
  "West Ham": "west_ham_united.png",
  "Bayer Leverkusen": "bayer_04_leverkusen.png",
  "Bayern Munich": "bayern_munich.png",
  "Inter Milan": "inter_milan.png",
  "Inter": "inter_milan.png",
  "Juventus": "juventus_fc.png",
  "Atalanta": "atalanta_bc.png",
  "Napoli": "ssc_napoli.png",
  "Roma": "as_roma.png",
  "Lazio": "ss_lazio.png",
  "Paris Saint-Germain": "paris_saint-germain.png",
  "PSG": "paris_saint-germain.png",
  "AS Monaco": "as_monaco.png",
  "Monaco": "as_monaco.png",
  "Brest": "stade_brestois_29.png",
  "Marseille": "olympique_marseille.png",
  "Lyon": "olympique_lyon.png",
  "Lille": "losc_lille.png",
  "Benfica": "sl_benfica.png",
  "Sporting CP": "sporting_cp.png",
  "Sporting": "sporting_cp.png",
  "Porto": "fc_porto.png",
  "Ajax": "ajax_amsterdam.png",
  "PSV Eindhoven": "psv_eindhoven.png",
  "Feyenoord": "feyenoord_rotterdam.png",
  "Club Brugge": "club_brugge.png",
  "Union Saint-Gilloise": "union_saint_gilloise.png",
  "FC Copenhagen": "fc_kobenhavn.png",
  "Copenhagen": "fc_kobenhavn.png",
  "Bodo/Glimt": "bodo_glimt.png",
  "Galatasaray": "galatasaray.png",
  "Olympiacos": "olympiacos.png",
  "Olympiakos": "olympiacos.png",
  "Qarabag": "fk_qarabag.png",
  "Slavia Prague": "slavia_prague.png",
  "Paphos": "pafos_fc.png",
  "Bologna": "bologna_fc_1909.png",
  "Girona": "girona_fc.png",
};

// Cricket IPL team mapping
export const cricketLogoMapping = {
  "Chennai Super Kings": "chennai_super_kings.png",
  "Mumbai Indians": "mumbai_indians.png",
  "Royal Challengers Bengaluru": "royal_challengers_bengaluru.png",
  "Royal Challengers Bangalore": "royal_challengers_bengaluru.png",
  "Kolkata Knight Riders": "kolkata_knight_riders.png",
  "Delhi Capitals": "delhi_capitals.png",
  "Punjab Kings": "punjab_kings.png",
  "Rajasthan Royals": "rajasthan_royals.png",
  "Sunrisers Hyderabad": "sunrisers_hyderabad.png",
  "Gujarat Titans": "gujarat_titans.png",
  "Lucknow Super Giants": "lucknow_super_giants.png",
};

/**
 * Get the local logo path for a team
 * @param {string} teamName - The team name to look up
 * @param {string} sport - 'football' or 'cricket'
 * @returns {string|null} - The path to the logo or null if not found
 */
export const getTeamLogoPath = (teamName, sport = 'football') => {
  const mapping = sport === 'cricket' ? cricketLogoMapping : footballLogoMapping;
  const logoFile = mapping[teamName];
  
  if (logoFile) {
    return `/assets/clubs/${sport}/${logoFile}`;
  }
  return null;
};
