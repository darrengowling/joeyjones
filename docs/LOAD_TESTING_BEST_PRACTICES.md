...
2. **Verify cleanup**:
   ...
   curl https://prod-auction-fix.preview.emergentagent.com/api/leagues | python -c "import sys, json; print(len(json.load(sys.stdin)))"
   ```
...
```bash
# Count leagues
curl https://prod-auction-fix.preview.emergentagent.com/api/leagues | python -c "import sys, json; print(f'Leagues: {len(json.load(sys.stdin))}')"
