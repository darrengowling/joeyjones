...
OR manually via API:
```bash
curl -X POST "https://stitch-dark-theme.preview.emergentagent.com/api/scoring/YOUR_LEAGUE_ID/ingest" \
...
```bash
# Via API
curl "https://stitch-dark-theme.preview.emergentagent.com/api/scoring/YOUR_LEAGUE_ID/leaderboard" | jq
