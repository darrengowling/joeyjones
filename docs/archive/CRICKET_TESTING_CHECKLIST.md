...
OR manually via API:
```bash
curl -X POST "https://fantasy-sports-bid.preview.emergentagent.com/api/scoring/YOUR_LEAGUE_ID/ingest" \
...
```bash
# Via API
curl "https://fantasy-sports-bid.preview.emergentagent.com/api/scoring/YOUR_LEAGUE_ID/leaderboard" | jq
