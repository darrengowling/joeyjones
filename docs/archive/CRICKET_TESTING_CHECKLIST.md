...
OR manually via API:
```bash
curl -X POST "https://fantasy-ux-pilot.preview.emergentagent.com/api/scoring/YOUR_LEAGUE_ID/ingest" \
...
```bash
# Via API
curl "https://fantasy-ux-pilot.preview.emergentagent.com/api/scoring/YOUR_LEAGUE_ID/leaderboard" | jq
