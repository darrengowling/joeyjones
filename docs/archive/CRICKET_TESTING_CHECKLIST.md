...
OR manually via API:
```bash
curl -X POST "https://stitch-fantasy.preview.emergentagent.com/api/scoring/YOUR_LEAGUE_ID/ingest" \
...
```bash
# Via API
curl "https://stitch-fantasy.preview.emergentagent.com/api/scoring/YOUR_LEAGUE_ID/leaderboard" | jq
