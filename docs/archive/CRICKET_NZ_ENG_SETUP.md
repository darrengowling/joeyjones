...
```bash
curl "https://fastbid-platform.preview.emergentagent.com/api/assets?sportKey=cricket" | jq '.pagination.total'
# Should return: 30
```

**Check Scoring Rules:**
```bash
curl "https://fastbid-platform.preview.emergentagent.com/api/sports/cricket" | jq '.scoringSchema'
