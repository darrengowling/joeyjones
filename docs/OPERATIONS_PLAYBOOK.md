...
**Frontend Status**:
```bash
curl -I https://prelaunch-fix.preview.emergentagent.com
...
curl -w "@-" -o /dev/null -s https://prelaunch-fix.preview.emergentagent.com/api/sports <<< "time_total: %{time_total}s"
```
...
curl -X POST https://prelaunch-fix.preview.emergentagent.com/api/auth/magic-link \
