...
**Frontend Status**:
```bash
curl -I https://prod-auction-fix.preview.emergentagent.com
...
curl -w "@-" -o /dev/null -s https://prod-auction-fix.preview.emergentagent.com/api/sports <<< "time_total: %{time_total}s"
```
...
curl -X POST https://prod-auction-fix.preview.emergentagent.com/api/auth/magic-link \
