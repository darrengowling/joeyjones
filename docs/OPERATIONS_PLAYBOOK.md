...
**Frontend Status**:
```bash
curl -I https://fastbid-platform.preview.emergentagent.com
...
curl -w "@-" -o /dev/null -s https://fastbid-platform.preview.emergentagent.com/api/sports <<< "time_total: %{time_total}s"
```
...
curl -X POST https://fastbid-platform.preview.emergentagent.com/api/auth/magic-link \
