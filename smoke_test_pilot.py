...
class SmokeTest:
    ...
    async def test_metrics_endpoint(self, session):
        ...
        try:
            async with session.get("https://leaguemaster-6.preview.emergentagent.com/api/metrics") as resp:
                if resp.status == 200:
    ...
    async def test_socketio_endpoint(self, session):
        ...
        try:
            async with session.get(
                "https://leaguemaster-6.preview.emergentagent.com/socket.io/?transport=polling"
            ) as resp:
