...
# Configuration
BASE_URL = "https://auctionpilot.preview.emergentagent.com/api"
SOCKET_URL = "https://auctionpilot.preview.emergentagent.com"
...
class EvertonBugFixesTester:
    ...
    def test_budget_reserve_enforcement(self) -> bool:
        ...
        result = self.test_api_endpoint("POST", f"/auction/{auction_id}/bid", bid_data)
        if "error" in result:
            self.log(f"£49m bid should have been accepted but was rejected: {result.get('text')}", "ERROR")
            return False
        ...
        # Test: Try to bid £2m on final team → SHOULD BE REJECTED (max bid £1m)
        self.log("Step 4: Testing £2m bid on final team (should be REJECTED - max bid £1m)...")
        
        bid_data = {
            "userId": self.test_data["commissioner_id"],
            "clubId": current_club["id"],
            "amount": 2000000.0  # £2m - should be rejected
        }
